...
const BookingSection = () => {
  ...
  const getStatus = (dateStr) => {
    ...
    return { text: t('bookingSection.available'), color: 'text-green-400', available: true };
  };
  ...
  // Show error modal when not enough spots - simplified without showing capacity numbers
  const showNoSpotsError = (date, requestedParticipants, availableSpots, customMessage = null) => {
    ...
    // Create WhatsApp message - simple, without capacity numbers
    const whatsappMessage = encodeURIComponent(
      `שלום חיליק,\n\n` +
      `ניסיתי לרשום ${requestedParticipants} אנשים לסיור בתאריך ${formattedDate}\n` +
      `אך מקבל/ת הודעה שאין מספיק מקומות.\n\n` +
      `האם יש אפשרות?\n\n` +
      `תודה!`
    );
    ...
    const message = customMessage || `מצטערים, אין מספיק מקומות פנויים לתאריך ${formattedDate}.\n\nניתן ליצור קשר דרך וואטסאפ לבדיקת אפשרויות.`;
  ...
  // Allow user to continue - actual capacity check happens when clicking "Continue"
