...
const Journey=()=>{const{t}=useTranslation();return <section id="journey" className="target-journey"><div className="target-section-heading"><p className="target-eyebrow">{t('journey.eyebrow')}</p><h2>{t('journey.title')}</h2><p>{t('journey.subtitle')}</p></div><div className="target-journey__grid">{stations.map((_,i)=><article key={keys[i]}><b>{String(i+1).padStart(2,'0')}</b><h3>{t(`journey.stations.${keys[i]}.title`)}</h3><p>{t(`journey.stations.${keys[i]}.desc`)}</p></article>)}</div></section>};
